...
print('tracen')
Parameterfile = os.path.join(Raytracer_Path, 'Parameter_Raytracer_neu.txt')
OUTfile = os.path.join(path_output_data,File_name+'_out')
...
#Excel File auslesen 
f = open(os.path.join(path_output_data,File_name +'_out.csv'), 'r') # open the file for reading
data_test = []
...
SPs = basic_data[:,1:4] # Schwerpunkte in um
n_points = len(SPs)
...
Phi_th = 3e8
delta_opt = 6e-3 # optische eindringtiefe in um
...
ablation_dist = Algorithmen.calc_ablat_dist(Is,Phi_th,delta_opt)
