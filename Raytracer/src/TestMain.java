...
public class TestMain {
    ...
    { 
        ...
        double[][] z = new double[30][30];
        x[0] = 0;
        y[0] = 0;
        
        for (int i =1;i<x.length;i++) { // set x,y
        	x[i] = x[i-1] + 0.1;
        	y[i] = y[i-1] + 0.1;
        }
        for (int i =1;i<x.length;i++) { // initialize z
        	for (int j =1;j<y.length;j++) {
            	z[i][j] = x[i]+y[j];
            }
        }
        double[][] zin = null;
		zin = read_array_from_file("profile1.csv");
        write_array_to_file("profile2.csv", zin);
        System.out.print(lin_interpolate2d(0.6, 0.6, zin));
        System.out.print(lin_interpolate2d(0., 0., zin));
    } 
